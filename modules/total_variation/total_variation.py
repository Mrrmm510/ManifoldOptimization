...
class TV(GradientDescent):
    ...
    def _horizontal_distance(self, X: np.ndarray) -> (np.ndarray, np.ndarray):
        ...
        Parameters
        ----------
        X : np.ndarray, shape = (height, width)
            A 2d image on sphere.

        Returns
        ----------
        ret_p : np.ndarray, shape = (height, width)
            Distance. Last column is zero vector.

        ret_n : np.ndarray, shape = (height, width)
            Distance. First column is zero vector.
        """
        ret_p = np.zeros_like(X)
        ret_n = np.zeros_like(X)
        dist = np.abs(self.manifold.log(X[:, :-1], X[:, 1:]))
        ret_p[:, :-1] = dist
        ret_n[:, 1:] = dist
        return ret_p, ret_n
    ...
    def _f(self, X: np.ndarray) -> float:
        ...
        """
        # first term
        dist = self.manifold.log(X, self.F)
    ...
    def _inpaint(self, F: np.ndarray) -> np.ndarray:
        ...
        Parameters
        ----------
        F : np.ndarray, shape = (height, width)
            A 2d image.
        ...
        X = F if self.mask is None else cv2.inpaint(F, 255 - np.uint8(self.mask > 0) * 255, 3, cv2.INPAINT_TELEA)

        X = X.astype(np.float32) / 255. if X.dtype == np.uint8 else X.astype(np.float32)
        return X
