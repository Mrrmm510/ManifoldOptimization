...
from ..manifolds.spheres import Spheres
...
class TVonSphere:
    ...
    def _horizontal_distance(self, X: np.ndarray) -> np.ndarray:
        ...
        dist = self.sphere2d.distance(X[:, :-1], X[:, 1:])
        ret_p[:, :-1] = dist
        ret_n[:, 1:] = dist
        return ret_p, ret_n
    ...
    def _horizontal_log(self, X: np.ndarray) -> np.ndarray:
        ret_p = np.zeros_like(X)
        ret_n = np.zeros_like(X)
        log_p = self.sphere2d.log(X[:, :-1], X[:, 1:])
        log_n = self.sphere2d.log(X[:, 1:], X[:, :-1])
        ret_p[:, :-1] = log_p
        ret_n[:, 1:] = log_n
        return ret_p, ret_n
    ...
    def _f(self, X: np.ndarray) -> float:
        ...
        """
        # first term
        dist = self.sphere2d.distance(X, self.F)
    ...
    def _df(self, X: np.ndarray) -> np.ndarray:
        ...
        """
        # first term
        d_dist = - self.sphere2d.log(X, self.F) if self.mask is None else - self.mask * self.sphere2d.log(X, self.F)
    ...
    def _step_size(self, X: np.ndarray, d: np.ndarray) -> float:
        ...
        t = self.initial_step
        f = self._f(X)
        while self._f(self.sphere2d.exp(X, t * d)) > f + self.armijo_param * t * g:
            t *= 0.5
    ...
    def transform(self, F: np.ndarray, mask: np.ndarray = None) -> np.ndarray:
        ...
        for _ in trange(self.max_iter):
            ...
            for _ in range(self.max_iter_gd):
                d = - self._df(X)
                step_size = self._step_size(X, d)
                X = self.sphere2d.exp(X, step_size * d)
